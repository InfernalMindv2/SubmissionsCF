...
void solve()
{
	...
	cin >> n >> d;
	/*
	علشان تقلل موضوع حساب mod في الدائره اعملها 
	3n --> left and right
	*/
	...
	for(ll i=1;i<=n+n+n;i++)
		pref[i]+=pref[i-1];
	/*
	مشكلتي اني وصلت ل90% من الفكره ولكن معرفتش اجيب ان
	لو حسبت كل واحده لوحديها بنفس المعادله هتبقي بنفس تأثير
	تداخل فتره 2d sliding window!
	2 * d - (summ of pref)
	*/
	ll ans=0;
	for(ll i=n;i<=n+n-1;i++)
